...
import { AxiosRequestConfigRetry, RequestOptions, Result, CustomAxiosRequestConfig } from '@/types/axios';
...
export class VAxios {
  ...
  private setupInterceptors() {
    ...
    this.instance.interceptors.request.use((config: CustomAxiosRequestConfig) => {
      // 如果忽略取消令牌，则不会取消重复的请求
      const ignoreCancel = config.requestOptions?.ignoreCancelToken ?? this.options.requestOptions?.ignoreCancelToken;
      if (!ignoreCancel) axiosCanceler.addPending(config);

      if (requestInterceptors && isFunction(requestInterceptors)) {
        config = requestInterceptors(config, this.options) as CustomAxiosRequestConfig;
      }
